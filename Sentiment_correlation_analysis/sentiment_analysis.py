...
def process_csv_tweets(crypto):
    df = pd.read_csv('csvs/' + crypto + '_tweets.csv')
    ...
    results_df.to_csv('csvs/' + crypto + '_sentiment.csv')
...
def analyze_relation(crypto):
    sentiment_data = pd.read_csv('csvs/' + crypto + '_sentiment.csv', index_col='Date', parse_dates=True).map(lambda x: 1 if x > 0.25 else (-1 if x < -0.1 else 0))
    price_data = pd.read_csv('csvs/' + crypto + '_data.csv', index_col='Date', parse_dates=True)
    merged_data = pd.merge(sentiment_data, price_data, on='Date')
    merged_data.to_csv('csvs/' + crypto + '_merged.csv')
...
def show_on_webpage(crypto):
    data = pd.read_csv('csvs/' + crypto + '_data.csv', index_col='Date', parse_dates=True)
    ...
    st.line_chart(pd.read_csv('csvs/' + crypto + '_sentiment.csv', index_col='Date', parse_dates=True)['compound'])
    st.markdown("#### Prices")
    st.line_chart(pd.read_csv('csvs/' + crypto + '_data.csv', index_col='Date', parse_dates=True)['Close'])
    st.markdown("#### Correlation")
    st.markdown("The correlation between the sentiment scores and the prices is " + "{:.3f}".format(pd.read_csv('csvs/' + crypto + '_merged.csv')['compound'].corr(pd.read_csv('csvs/' + crypto + '_merged.csv')['Close'])))

def execute(crypto):

    sentiment_file = 'csvs/' + crypto + '_sentiment.csv'
    merged_file = 'csvs/' + crypto + '_merged.csv'
